...
async function verifyV2Compatibility(api, endpointPath, primaryKey, aliasKey) {
  ...
  if (items.length > 0) {
    const item = items[0];
    assert(Object.prototype.hasOwnProperty.call(item, "name"), `${endpointPath} v2 item missing top-level name`);
    assert(
      Object.prototype.hasOwnProperty.call(item, "last_name"),
      `${endpointPath} v2 item missing top-level last_name`
    );
  }

  assert(first.data?.body?.paging && typeof first.data.body.paging === "object", `${endpointPath} v2 missing paging`);
  assert(
    Object.prototype.hasOwnProperty.call(first.data.body.paging, "next_cursor"),
    `${endpointPath} v2 paging missing next_cursor`
  );

  console.log(`[pass] ${endpointPath} v2 items=${items.length} paging=yes`);
}
