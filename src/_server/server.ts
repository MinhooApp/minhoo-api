...
class Server {
  ...
  private socketRedisPub: any | null = null;
  private socketRedisSub: any | null = null;
  ...
  constructor(options: Options) {
    ...
    void this.configureSocketScaling();
    this.middlewares();
    this.dbConnection();
    this.configure();
  ...
  async configureSocketScaling() {
    const redisUrl = String(
      process.env.SOCKET_REDIS_URL ?? process.env.REDIS_URL ?? ""
    ).trim();
    if (!redisUrl) {
      return;
    }
    ...
    try {
      // Lazy runtime import so local/dev keeps working without redis packages.
      const redisPkg = require("redis");
      const adapterPkg = require("@socket.io/redis-adapter");
      const createClient =
        typeof redisPkg?.createClient === "function"
          ? redisPkg.createClient
          : null;
      const createAdapter =
        typeof adapterPkg?.createAdapter === "function"
          ? adapterPkg.createAdapter
          : null;

      if (!createClient || !createAdapter) {
        console.warn(
          "[socket] Redis adapter requested but redis/@socket.io/redis-adapter is unavailable."
        );
        return;
      }
      ...
      pubClient.on("error", (error: any) => {
        console.log("[socket] redis pub error:", error?.message ?? error);
      });
      subClient.on("error", (error: any) => {
        console.log("[socket] redis sub error:", error?.message ?? error);
      });

      await Promise.all([pubClient.connect(), subClient.connect()]);
      this.io.adapter(createAdapter(pubClient, subClient));
      this.socketRedisPub = pubClient;
      this.socketRedisSub = subClient;

      console.log("[socket] Redis adapter enabled for horizontal scaling.");
    } catch (error) {
      console.warn("[socket] Redis adapter disabled:", error);
    }
  }
